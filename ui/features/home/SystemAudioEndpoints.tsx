...
const STATUS_KEYS: Record<SystemAudioEndpointStatus, string> = {
  ...
  unsupported: 'systemAudio.componentNotInstallable',
};
...
export function SystemAudioEndpoints({
  ...
  const [physicalOutputBusy, setPhysicalOutputBusy] = useState(false);
  const label = backendLabel(backendKind);
  const missingVbCable = audioRouteChoice === 'legacy-automatic'
    && backendStatus === 'component-required'
    && backendKind !== 'apo';
  const physicalApoUnavailable = audioRouteChoice === 'physical-apo' && backendStatus === 'component-required';
  const usingRelay = backendKind === 'vb-cable-relay' || backendKind === 'voxveil-cable-relay';
  const selectPhysicalOutput = async (endpointId: string) => {
    if (busy || physicalOutputBusy) return;
    setPhysicalOutputBusy(true);
    try {
      await onSelectPhysicalOutput(endpointId);
    } finally {
      setPhysicalOutputBusy(false);
    }
  };
    ...
    <section className="system-audio-panel" aria-label={t('systemAudio.title')}>
      <div className="system-audio-header">
        ...
        <div className="system-audio-actions">
          <button className="action-button is-subtle" type="button" disabled={busy || physicalOutputBusy || Boolean(installBusyId)} onClick={onRefresh}>
      ...
      <div className="system-audio-row">
        <div className="system-audio-identity">
          ...
          {physicalApoUnavailable && <small>{t('systemAudio.physicalApoUnavailable')}</small>}
      ...
      {usingRelay && physicalOutputs.length > 0 && (
        <label className="system-audio-row">
          ...
          <select
            ...
            disabled={busy || physicalOutputBusy}
            value={selectedPhysicalOutputId ?? ''}
            onChange={(event) => void selectPhysicalOutput(event.currentTarget.value)}
          >
