...
export function EngineScreen({ model }: { model: VoxveilModel }) {
    ...
    <section className="screen" aria-labelledby="engine-title">
      ...
      <div className="engine-list">
        ...
        <article className={model.state.engine === 'ai' ? 'engine-row is-active' : 'engine-row'}>
          <div><strong>{t('engine.ai')}</strong><span>{t('engine.aiDescription')}</span></div><span className="mono">{t('engine.noModel')}</span>
        </article>
      </div>
