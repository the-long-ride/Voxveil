...
param(
  ...
  [Parameter(Mandatory = $true)]
  [ValidateSet('controlled', 'natural-mix')]
  [string]$Tier,
...
if ($LASTEXITCODE -ne 0) {
  throw "ffprobe.exe failed while reading the evaluation input with exit code $LASTEXITCODE."
}
$probeText = (($probeOutput | ForEach-Object { "$_".Trim() } | Where-Object { $_ }) -join '')
[int]$sourceSampleRate = 0
if (-not [int]::TryParse($probeText, [ref]$sourceSampleRate) -or $sourceSampleRate -le 0) {
  throw "Could not determine the source sample rate from ffprobe output: $probeText"
}
if ($sourceSampleRate -ne $SampleRate) {
  throw "Evaluation input sample rate is $sourceSampleRate Hz but $SampleRate Hz was requested. Release-acceptance evidence must use native-rate input rather than resampled material."
}
...
$strongRaw = Join-Path $renders "$baseName-strong.f32"
$strongWav = Join-Path $renders "$baseName-strong.wav"
...
$strongBytes = (Get-Item $strongRaw).Length
if ($musicBytes -ne $fixtureBytes -or $balancedBytes -ne $fixtureBytes -or $strongBytes -ne $fixtureBytes) {
  throw "Latency-compensated render size mismatch: input=$fixtureBytes music=$musicBytes balanced=$balancedBytes strong=$strongBytes."
}

foreach ($render in @(
  @{ Raw = $musicRaw; Wav = $musicWav },
  @{ Raw = $balancedRaw; Wav = $balancedWav },
  @{ Raw = $strongRaw; Wav = $strongWav }
)) {
  Invoke-Checked $ffmpeg @(
    '-v', 'error',
    '-f', 'f32le',
...
$strongSha256 = Get-Sha256 $strongRaw
...
$evidence = [ordered]@{
  ...
  source = [ordered]@{
    ...
    nativeSampleRate = $sourceSampleRate
    sha256 = $sourceSha256
  }
  input = [ordered]@{
  ...
  renders = [ordered]@{
    ...
    strong = [ordered]@{
      rawFile = "renders/$baseName-strong.f32"
      wavFile = "renders/$baseName-strong.wav"
      sha256 = $strongSha256
      metrics = Get-RawMetrics $strongRaw
    }
  }
...
Write-Host "Strong SHA-256: $strongSha256"
