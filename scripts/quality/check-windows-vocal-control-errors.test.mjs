...
test('controller and Tauri command preserve vocal-control errors', () => {
  ...
  assert.match(controller, /backend\.set_vocal_level\(value\)\?/);
  assert.match(controller, /playback\.set_vocal_level\(value\)\?/);
  assert.match(controller, /AudioPlaybackStatus::Playing\s*\|\s*AudioPlaybackStatus::Paused/);
