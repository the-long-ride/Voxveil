...
test('Windows package fails if a required system-audio script is missing', () => {
  const start = buildScript.indexOf('$packageScripts = @(');
  ...
  assert.match(staging, /if \(-not \$ApoOnly\)[\s\S]*install-staged-virtual-driver\.ps1[\s\S]*uninstall-staged-virtual-driver\.ps1/i);
  assert.match(staging, /foreach \(\$script in \$packageScripts\)/);
...
test('APO-only variant excludes the virtual driver and marks it not applicable', () => {
  assert.match(buildScript, /if \(-not \$ApoOnly\)[\s\S]*VoxveilVirtualAudioDevice\.vcxproj/i);
  assert.match(buildScript, /if \(-not \$ApoOnly\)[\s\S]*voxveil-virtual-device\.exe/i);
  assert.match(buildScript, /packageVariant\s*=\s*if \(\$ApoOnly\) \{ 'apo-only' \}/i);
  assert.match(buildScript, /virtualDriverComponent\s*=\s*\[ordered\]@\{[\s\S]*applicability\s*=\s*if \(\$ApoOnly\) \{ 'not-applicable' \}/i);
  assert.match(buildScript, /No virtual-driver package, helper, installer, or lifecycle scripts are included/i);
});
