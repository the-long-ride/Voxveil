...
test('malformed Windows audio preference JSON falls back without swallowing read failures', () => {
  ...
  assert.match(load, /if !persisted_json\.is_object\(\)[\s\S]{0,100}Ok\(WindowsAudioPreferences::default\(\)\)/);
  assert.match(load, /decode_preferences\(&bytes\)[\s\S]{0,120}map_err/);
