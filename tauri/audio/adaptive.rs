...
use super::{
    ...
    quality::{QualityTier, quality_tier},
};
...
fn ai_for_tier(tier: QualityTier, conditions: RuntimeConditions) -> RuntimeEngine {
    ...
    if conditions.underrun_risk
        || conditions.thermal_pressure
        || conditions.cpu_pressure
        || conditions.battery_saver
    {
    ...
    match tier {
        ...
        QualityTier::HighQuality if accelerator_available(conditions) => {
...
mod tests {
    ...
    fn explicit_dsp_never_selects_ai() {
        assert_eq!(
            select_runtime_engine(ProcessingEngineKind::Dsp, 100, available()),
            RuntimeEngine::ClassicDsp
        );
    }
    ...
    fn auto_uses_dsp_when_ai_is_missing_or_latency_is_priority() {
        ...
        assert_eq!(
            select_runtime_engine(ProcessingEngineKind::Auto, 10, available()),
            RuntimeEngine::ClassicDsp
        );
    }
    ...
    fn high_quality_requires_ai_and_an_accelerator() {
        assert_eq!(
            select_runtime_engine(ProcessingEngineKind::Auto, 90, available()),
            RuntimeEngine::HighQualityAi
        );
        let cpu_only = RuntimeConditions {
            ai_available: true,
            ..Default::default()
        };
        assert_eq!(
            select_runtime_engine(ProcessingEngineKind::Auto, 90, cpu_only),
    ...
    fn pressure_degrades_but_never_exceeds_user_quality() {
        let pressured = RuntimeConditions {
            thermal_pressure: true,
            ..available()
        };
        assert_eq!(
            select_runtime_engine(ProcessingEngineKind::Auto, 90, pressured),
            RuntimeEngine::FastAi
        );
        assert_eq!(
            select_runtime_engine(ProcessingEngineKind::Auto, 30, available()),
            RuntimeEngine::FastAi
        );
    }
}
