...
#[path = "controller_playback.rs"]
mod playback;

use voxveil_types::{
    AudioPlaybackSnapshot, AudioPlaybackStatus, ClassicSuppressionProfile, ProcessingBackendStatus,
    WindowsInterceptionPolicy,
};
...
pub struct ProcessingController {
    ...
    playback: Mutex<voxveil_windows_audio::OwnedPlayback>,
}
...
impl Default for ProcessingController {
    fn default() -> Self {
        Self {
            ...
            playback: Mutex::new(voxveil_windows_audio::OwnedPlayback::default()),
        }
    }
}
...
impl ProcessingController {
    ...
    pub fn set_vocal_level(&self, value: u8) -> Result<(), String> {
        ...
        if matches!(
            ...
            AudioPlaybackStatus::Playing | AudioPlaybackStatus::Paused
        ) {
            playback.set_vocal_level(value)?;
        }
        Ok(())
    }
    ...
    pub fn set_classic_suppression_profile(
        ...
        #[cfg(target_os = "windows")]
        {
            let mut backend = self
                ...
                .lock()
                .map_err(|_| "Windows audio backend lock is poisoned".to_string())?;
            backend.set_classic_suppression_profile(profile)?;
        }
        #[cfg(not(target_os = "windows"))]
        {
            let _ = profile;
    ...
    pub fn set_interception_policy(
        ...
        if policy != WindowsInterceptionPolicy::Disabled {
            self.stop_owned_playback()?;
        }
        #[cfg(target_os = "windows")]
        {
            let mut backend = self
                ...
                .lock()
                .map_err(|_| "Windows audio backend lock is poisoned".to_string())?;
            return Ok(from_windows_probe(backend.set_interception_policy(policy)));
        }
        #[cfg(not(target_os = "windows"))]
        {
            let _ = policy;
            Ok(BackendSnapshot {
                status: ProcessingBackendStatus::Unsupported,
                detail: Some("Windows audio routing is unavailable on this platform".into()),
                backend_kind: None,
                physical_output: None,
                physical_output_endpoint_id: None,
                per_app_available: false,
    ...
    pub fn set_physical_output(
        ...
        let active_output = self.playback_snapshot().output_endpoint_id;
        if active_output.as_deref() != endpoint_id.as_deref()
            && matches!(
                self.playback_snapshot().status,
...
fn faulted_snapshot() -> BackendSnapshot {
    BackendSnapshot {
        ...
        detail: Some("Windows audio backend lock is poisoned".into()),
        backend_kind: None,
        physical_output: None,
        physical_output_endpoint_id: None,
        per_app_available: false,
