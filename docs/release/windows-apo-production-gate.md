...
Before a release build uses those files, `scripts/windows/verify-signed-apo-package.ps1` must verify both INF files, both Microsoft-trusted catalogs, each catalog's package membership, the fixed CLSID/CAPX/Extension identities, x64 APO architecture, the APO DLL's Authenticode signature required by the INF `SignatureAttributes.PETrust` declaration, and the embedded `VOXVEIL_BUILD_COMMIT=<40-hex>` marker. The marker is compiled into `VoxveilApo.dll` from the exact Git HEAD used by `build-windows.ps1` and survives Microsoft signing. `scripts/windows/stage-signed-apo-package.ps1` copies only those verified files and records their hashes plus exact signer subjects/thumbprints in `apo-verification.json`. Endpoint discovery requires that complete provenance before offering production installability, and the elevated installer revalidates the current signatures against the recorded subjects/thumbprints before PnP mutation.
...
Build the unsigned APO submission input from the exact release checkout so `VoxveilApo.dll` carries that checkout's commit marker. For a release package, set `VOXVEIL_SIGNED_APO_DIR` to the verified returned package before running `npm run build:windows`. The build and signed-APO stager both reject a returned package whose embedded commit does not equal the current Git HEAD. The build script does not copy that directory directly; it invokes the verifier/stager. When the variable is unset, the locally built APO DLL/INF remain development artifacts and the production installer intentionally lacks the catalogs/Extension INF/verifier marker needed to install them as production packages.
...
- Production UAC launch binds the privileged installer script to bytes embedded in the Tauri executable, hashes the exact endpoint-descriptor bytes across the elevation handoff, resolves Windows PowerShell from the OS-reported system directory instead of inherited environment variables, and verifies build-time SHA-256 identities for `discover-system-audio-endpoints.ps1`, `voxveil-control.exe`, and `VoxveilControl.dll` before privileged mutation. The elevated script independently resolves `[Environment]::SystemDirectory` and rechecks those helper hashes immediately before use.
...
- Legacy/TestSign state created before certificate thumbprints were recorded is not repaired in place when owned packages still exist. Uninstall the recorded APO packages first. The uninstaller must not guess certificate ownership by subject; any old `Voxveil Development APO` trust entries from that legacy state require manual review/cleanup.
- Legacy runtime-interface development installs separately persist whether `attach-effects` actually succeeded. Uninstall runs `detach-effects` only when that attachment is recorded, then checkpoints the cleared attachment before deleting packages so partial/early TestSign failures remain recoverable.
