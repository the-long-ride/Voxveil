...
Strong is opt-in and trades centered-instrument preservation for deeper center attenuation. It starts from a `0.03162278` (-30 dB) center floor with Balanced's band and transient settings. A smoothed whole-frame side-to-mid power ratio protects near-mono material: the floor rises toward `0.5` (-6 dB) when side energy is at or below `0.001` of mid energy, then ramps back toward -30 dB by a ratio of `0.01`. This guard preserves audibility; it cannot identify vocals in mono material. Strong can still reduce centered instruments and does not guarantee a vocal-free result.

The Strong thresholds are engineering starting points, not a release-quality claim. The controlled and natural-mix evaluation in `docs/testing/classic-dsp-evaluation.md` must compare Balanced and Strong at matched levels and report accompaniment damage before Strong is promoted beyond an opt-in profile.
...
## Native APO implementation boundary

The current APO callback uses a bounded, allocation-free band-limited mid suppressor. The relay and owned-file paths use the Rust STFT suppressor. Their algorithms and reported processing delay are not yet in parity. Strong is carried through shared-control ABI v4; the APO applies a bounded Strong floor and near-mono guard on top of its band-limited fallback. Do not describe the native APO and Rust paths as equivalent until the shared-DSP/latency work and parity qualification are complete.

The APO must never return to full-band hard center cancellation. A spectral APO port or bridge must prepare all state outside `APOProcess`, handle silence and bypass at a fixed delay, and pass native timing, allocation, import-residency, and qualification checks before replacing this fallback.
