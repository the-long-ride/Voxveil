...
## User-selected audio routes

Voxveil exposes two explicit no-virtual-device choices:

- **Physical output APO** processes supported Windows playback streams that traverse an endpoint with Voxveil's APO installed. It does not create or require a virtual audio endpoint, but it does install a signed Windows audio component on a compatible physical endpoint. APO availability remains endpoint- and graph-dependent; unsupported formats and modes must not be reported as processed.
- **Play files in Voxveil** opens local MP3, WAV, FLAC, and Ogg Vorbis files, applies Classic DSP, and renders only Voxveil's output to a physical endpoint. It does not intercept other applications and does not install an audio component. Mono is duplicated to stereo; multichannel files are rejected.

Selecting the owned-file route disables system interception and requires the built-in DSP engine. Its file path stays in the native playback worker; UI state exposes the file name, endpoint, format timing, and playback status only. Legacy automatic interception remains available for existing setups.
