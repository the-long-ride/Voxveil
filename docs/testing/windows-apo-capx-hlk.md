...
| Uninstall rerun finds a recorded APO/Extension INF already absent, or non-3010 delete returns after the INF disappeared | treat as interrupted/ambiguous deletion, remove only that INF from owned-package state, persist it as `pendingRemovedInfName` with the current boot marker, force restart, then prove post-reboot absence before continuing |
| Any lifecycle state checkpoint | write JSON to a same-directory temporary file and atomically replace the prior state; a torn write must not replace the last complete ownership record |
...
| Legacy/TestSign state with owned packages but no recorded certificate thumbprint | repair is rejected; scoped uninstall removes only recorded package/FX state and warns that old trust-store certificates require manual review instead of broad subject-based deletion |
| Production UAC helper integrity | endpoint descriptor SHA-256 survives the elevation handoff; installer script bytes match the executable-embedded digest; discovery/control EXE/control DLL match build-time SHA-256 values both before and after elevation; Windows PowerShell is invoked by absolute System32 path |
| Legacy runtime interface mutation | topology/audio interface paths must resolve to the exact expected PnP instance **and** the expected topology/audio interface-class GUID before any FX key is opened |
