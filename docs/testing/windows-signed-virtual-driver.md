...
Before privileged mutation, the installer re-hashes the retained `submission-manifest.json` against `verification.json` and revalidates its exact commit, architecture, pinned SysVAD provenance, returned INF/SYS identity, and recorded unsigned CAT/PDB hashes. The installer then opens the staged INF/CAT/SYS and packaged `voxveil-virtual-device.exe` under verified read locks against `virtual-driver/verification.json`. Those locks deny write/delete sharing until the privileged lifecycle finishes, closing both package hash-to-PnPUtil and helper hash-to-execution replacement windows. The installer also verifies the fixed Voxveil driver/interface identity and requires the current Microsoft catalog signer subject **and certificate thumbprint** to match the staged manifest. The uninstaller uses the same locked helper verification before any recorded devnode operation. It resolves native processor architecture through `Win32_Processor` and rejects a package/OS architecture mismatch before creating or reusing the root devnode. Only then does it ensure exactly one `Root\VoxveilVirtualAudio` devnode through `voxveil-virtual-device.exe` and invoke `pnputil /add-driver ... /install`. It resolves the installed driver from the exact returned device instance and records both that instance ID and one published INF in `virtual-driver/virtual-driver-install-state.json`.
