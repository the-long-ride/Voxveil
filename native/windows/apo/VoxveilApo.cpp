    ...
    if (state_ != nullptr && !initializeForDiscoveryOnly_) {
        ...
        if (voxveil::ShouldProcess(appEnabled, systemEffectEnabled, vocal) && channels >= 2) {
            ...
            for (UINT32 frame = 0; frame < frames; ++frame) {
                const FLOAT32* samples = inputSamples + (static_cast<size_t>(frame) * channels);
                const double mid = (static_cast<double>(samples[0]) + samples[1]) * 0.5;
                const double side = (static_cast<double>(samples[0]) - samples[1]) * 0.5;
                midPower += mid * mid;
                sidePower += side * side;
            }
            const double rawRatio = sidePower / (midPower + 1.0e-9);
            const double boundedRatio = rawRatio > 1.0 ? 1.0 : rawRatio;
            const FLOAT32 frameRatio = boundedRatio >= 0.0
                ? static_cast<FLOAT32>(boundedRatio)
                : 0.0f;
            const FLOAT32 ratioSmoothing = (static_cast<FLOAT32>(frames) / 960.0f) < 1.0f
                ? (static_cast<FLOAT32>(frames) / 960.0f)
                : 1.0f;
            smoothedSideToMidRatio_ += ratioSmoothing * (frameRatio - smoothedSideToMidRatio_);
            const FLOAT32 bandGain = voxveil::CenterBandGain(vocal, profile, smoothedSideToMidRatio_);
