...
int wmain() {
    ...
    const auto strong = ClassicSuppressionProfile::Strong;
    ...
    assert(MinimumCenterGain(strong) < MinimumCenterGain(balanced));
    assert(MinimumCenterGain(strong, 0.0f) == 0.5f);
    assert(MinimumCenterGain(strong, 0.01f) < 0.032f);
    ...
    assert(NormalizeProfile(static_cast<LONG>(strong)) == strong);
