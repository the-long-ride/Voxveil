use super::control::{parse_loaded_instances, parse_loaded_instances_required};
use super::support::{
    decide_backend, decide_backend_with_policy, profile_control_value,
    should_fail_closed_after_probe, should_sync_apo_after_relay,
};
use super::*;
use crate::device::{RelayReadiness, WindowsInterceptionKind};
use voxveil_types::WindowsInterceptionPolicy;
...
fn profile_control_values_match_the_shared_wire_contract() {
    ...
    assert_eq!(
        profile_control_value(ClassicSuppressionProfile::Strong),
        "strong"
    );
}
...
fn loaded_apo_has_priority_over_virtual_relay() {
    ...
    assert_eq!(
        ...
        Some(WindowsInterceptionKind::Apo)
    );
    assert_eq!(decision.readiness, RelayReadiness::Ready);
}

#[test]
fn physical_apo_only_never_selects_a_running_virtual_relay() {
    let cable = vb_cable(true);
    let physical = physical("speakers", true);
    let decision = decide_backend_with_policy(
        WindowsInterceptionPolicy::PhysicalApoOnly,
        false,
    ...
    assert_eq!(decision.route.interception, None);
    assert_eq!(decision.route.source_endpoint_id, None);
    assert!(decision.detail.as_deref().unwrap().contains("physical APO"));
    assert!(!decision.detail.as_deref().unwrap().contains("VB-CABLE"));
}

#[test]
fn disabled_interception_never_reports_a_loaded_apo_as_ready() {
    let physical = physical("speakers", true);
    let decision = decide_backend_with_policy(
        WindowsInterceptionPolicy::Disabled,
        true,
        None,
        Some(&physical),
        None,
    ...
    assert_eq!(decision.readiness, RelayReadiness::Unsupported);
    assert_eq!(decision.route.interception, None);
}
...
fn only_ready_probe_decisions_may_keep_processing_active() {
    ...
    assert!(should_fail_closed_after_probe(
