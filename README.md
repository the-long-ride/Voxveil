...
The staged output is written to `dist/windows-x64/Voxveil` by default. The manual builder runs Windows-audio Rust tests, repository quality tests, and the native APO CAPX policy test, then builds the native control/APO projects and the Tauri executable and writes SHA-256 checksums.
